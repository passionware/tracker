/**
 * Serialized Cube View with Selection Support
 *
 * A wrapper around CubeViewWithSelection that automatically handles ListView rendering for raw data
 * using the serialized listView configuration.
 */

import { rd } from "@passionware/monads";
import { CubeViewWithSelection } from "./CubeViewWithSelection";
...
export interface SerializedCubeViewWithSelectionProps {
...
/**
 * SerializedCubeViewWithSelection component
 *
 * A wrapper around CubeViewWithSelection that automatically renders raw data using ListView
 * based on the serialized configuration's listView setup.
 */
export function SerializedCubeViewWithSelection({
  state,
  serializedConfig,
  className,
  maxInitialDepth = 0,
  enableZoomIn = true,
}: SerializedCubeViewWithSelectionProps) {
  return (
